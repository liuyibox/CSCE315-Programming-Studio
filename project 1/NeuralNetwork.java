...
class NeuralNetwork {
    ...
    final int OUTPUT_SIZE = 26;
    final int INPUT_SIZE = 35;
    	...
    	if(numLayers == 1) {
    		for(int i = 0; i < INPUT_SIZE; i++) {                              // For every input,
    			for(int j = 0; j < OUTPUT_SIZE; j++) {                         // Distribute its output weights.           
    				weights1[i][j] = ((float)r.nextDouble()-0.5f)*0.01f;       // [inputs][Outputs]
    			}
    		}
    	} 
    	else {				 																				
    		for(int i = 0; i < INPUT_SIZE; i++) {							   // For every input,    
    			for(int j = 0; j < HIDDEN_SIZE; j++) {						   // Distribute its output weights.
    				weights1[i][j] = ((float)r.nextDouble()-0.5f)*0.01f;	   // Distribute: input_weights -> HiddenLayer
    			}
    		}
    		for(int j = 0; j < HIDDEN_SIZE; j++) {							   // For every hidden neuron,
    			for(int k = 0; k < OUTPUT_SIZE; k++) {						   // Distribute its output weights.
    				weights2[j][k] = ((float)r.nextDouble()-0.5f)*0.01f;	   // Distribute: hidden_weights -> OutputLayer
    			}
			}   		
    	}
   }
